...
export default function Menu() {
  const { isLoggedIn, userEmail, userRole, logout } = useAuth(); // 2. PEGUE OS DADOS
        ...
        {isLoggedIn ? (
          ...
          <div className="menu-auth">
            ...
            {useAuth().userRole === 'admin' && (
              <> {/* Fragmento para agrupar os links de admin */}
                <Link to="/admin/vacinas">Ger. Vacinas</Link>
                <Link to="/admin/unidades">Ger. Unidades</Link>
              </>
            )}
